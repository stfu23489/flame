...
genKeysBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    yourPub.value = encodeBase64ToCustom(toBase64(mlkemPub), encoderAlphabet) + "||" + encodeBase64ToCustom(toBase64(faPub), encoderAlphabet);
    yourPriv.value = encodeBase64ToCustom(toBase64(mlkemPriv), encoderAlphabet) + "||" + encodeBase64ToCustom(toBase64(faPriv), encoderAlphabet);
...
exportBtn.addEventListener('click', () => {
  ...
  const encodedKeys = [
    encodeBase64ToCustom(toBase64(mlkemPub), encoderAlphabet),
    encodeBase64ToCustom(toBase64(faPub), encoderAlphabet),
    encodeBase64ToCustom(toBase64(mlkemPriv), encoderAlphabet),
    encodeBase64ToCustom(toBase64(faPriv), encoderAlphabet)
  ].join("||");
  impExp.value = encodedKeys;
...
importBtn.addEventListener('click', () => {
  ...
  if (parts.length !== 4) return showAlert("Import format: MLKemPub||FalconPub||MLKemPriv||FalconPriv", true);
  
  const decodedParts = parts.map(p => fromBase64(decodeCustomToBase64(p, encoderAlphabet)));
  const [kp, fp, ks, fs] = decodedParts;

  if (!kp || !fp || !ks || !fs) return showAlert("Invalid custom-encoded keys.", true);
  mlkemPub = kp; faPub = fp; mlkemPriv = ks; faPriv = fs;
...
encBtn.addEventListener('click', async () => {
  ...
  const pr = rec.split("||");
  if (pr.length !== 2) return showAlert("Recipient key format: custom_encoded_MLKemPub||custom_encoded_FalconPub", true);
  
  const rkp = fromBase64(decodeCustomToBase64(pr[0], encoderAlphabet));
  const rfp = fromBase64(decodeCustomToBase64(pr[1], encoderAlphabet));
  if (!rkp || !rfp) return showAlert("Invalid custom-encoded recipient pub keys.", true);
...
decBtn.addEventListener('click', async () => {
  ...
  if (pp.length !== 2 || pu.length !== 2) return showAlert("Your keys must be custom-encoded MLKem||Falcon", true);
  
  const sK = fromBase64(decodeCustomToBase64(pp[0], encoderAlphabet));
  const sF = fromBase64(decodeCustomToBase64(pp[1], encoderAlphabet));
  const pK = fromBase64(decodeCustomToBase64(pu[0], encoderAlphabet));
  const pF = fromBase64(decodeCustomToBase64(pu[1], encoderAlphabet));
  if (!sK || !sF || !pK || !pF) return showAlert("Invalid custom-encoded data in your keys.", true);
